...
def get_map(vars, B, xdata, t_max):
    ...
    fluxs = np.zeros((len(ts), len(Egs), len(thetas))) # Initialize the fluxes array
    for n, t in enumerate(ts):
        Model = LeptonicModel(B, Q0, A, Ec, t, D=2000)
        _fluxs = Model(Egs, thetas=thetas, get_map=True)
        fluxs[n] = _fluxs * bin_widths[n] / (t_max * 3.154e7)
    ind_1TeV = np.where(xdata >= 1e12)[0][0]
    ind_10TeV = np.where(xdata >= 1e13)[0][0]
    for n in range(len(fluxs)):
        if n != 0 and n != len(fluxs)-1:
            a = fluxs[n-1]
            b = fluxs[n]
            c = fluxs[n+1]
            if (np.sum(b[ind_10TeV]) > np.sum(a[ind_10TeV]) and np.sum(b[ind_10TeV]) > np.sum(c[ind_10TeV])) or (np.sum(b[ind_1TeV]) > np.sum(a[ind_1TeV]) and np.sum(b[ind_1TeV]) > np.sum(c[ind_1TeV])):
                fluxs[n] = (a + c) / 2
                    
    return fluxs # 1/eV/cm2/s/sr
...
vars = (Q0, A, Ec)
print('Getting map with:', vars)
map = get_map(vars, B, xdata, t_max)

np.save(f'map_Crab_A{A*10:.0f}_Ec{Ec*10:.0f}_B{B:.0f}.npy', map) # 1/eV/cm2/s/sr
